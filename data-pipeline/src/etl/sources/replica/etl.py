...
class ReplicaETL:
    ...
    def run(self):
        ...
        for filename in geojson_filename:
            ...
            segments_df = self._run_for_network_segments(gdf, area_name, self.schema_df)
    ...
    def _run_for_trips(self, gdf: geopandas.GeoDataFrame, area_name: str, schema_df: pandas.DataFrame) -> pandas.DataFrame:
        ...
        schema_df = self.schema_df[schema_df['table_name'].str.endswith('trip')]
        ...
        if schema_df is not None and not schema_df.empty:
            ...
            # Set full_table_path be equal to the table_name column in the schema_df
                ...
                if not table_df.empty:
                    ...
                    trip_type_match = re.search(r'_(thursday|saturday)_trip', table_name) #this regex will match the trip type
                    # If no match is found, default to 'other_trip'
                    trip_type = trip_type_match.group(0)[1:] if trip_type_match else 'other_trip' #this will get the trip type from the regex match
                    # Build directory path dynamically
                    area_folder_path = os.path.join(self.folder_path, area_name, trip_type)
                    ...
                    output_path = os.path.join(area_folder_path, output_filename)
    ...
    def _run_for_network_segments(self, gdf: geopandas.GeoDataFrame, area_name: str, schema_df: pandas.DataFrame) -> pandas.DataFrame:
        ...
        if schema_df is not None and not schema_df.empty:
            ...
            for _, row in gdf.iterrows():
                rows_str = str((row['name'], row['geometry_wkt'])) # this is a tuple for each row
                ...
                '''
                segments_df = pandas_gbq.read_gbq(segments_query, project_id=self.project_id, dialect='standard')
                result_dfs.append(segments_df)
                #Build output path with both area_name and table_name
                safe_table_name = table_name.replace('.', '_')
                output_filename = f"{area_name}_{safe_table_name}.json"
                segment_folder_path = os.path.join(self.folder_path, area_name, "network_segments")
                os.makedirs(segment_folder_path, exist_ok=True)
                output_path = os.path.join(segment_folder_path, output_filename)
    ...
    def _run_for_pop_(self, gdf: geopandas.GeoDataFrame, area_name: str, schema_df: pandas.DataFrame) -> pandas.DataFrame:
        ...
        schema_df = schema_df[schema_df['table_name'].str.endswith('population')]
        ...
        if schema_df is not None and not schema_df.empty:
            ...
            for _, row in gdf.iterrows():
                ...
                '''
                population_df = pandas_gbq.read_gbq(pop_query, project_id=self.project_id, dialect='standard')
                result_dfs.append(population_df)
                #Build output path with both area_name and table_name
                safe_table_name = table_name.replace('.', '_')
                output_filename = f"{area_name}_{safe_table_name}.json"
                pop_folder_path = os.path.join(self.folder_path, area_name, "population")
