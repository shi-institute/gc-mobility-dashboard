...
def source_runner():
    ...
    # get the areas that correspond to the census tracts
    area_json_paths = list(
        sorted(Path('./input/replica_interest_area_polygons').glob('*.geojson')))
    geo = CensusIntersectAreasETL(area_json_paths).run()
